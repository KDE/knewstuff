...
Entry AtticaProvider::entryFromAtticaContent(const Attica::Content &content)
{
    ...
    for (const Attica::DownloadDescription &desc : descs) {
        auto info = std::make_unique<DownloadLinkInformationV2Private>(
            DownloadLinkInformationV2Private{.name = desc.name(),
        ...
        entry.d->mDownloadLinkInformationList.append(DownloadLinkInformationV2(info));
    }
